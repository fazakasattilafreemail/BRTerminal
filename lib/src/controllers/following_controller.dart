...
class FollowingController extends ControllerMVC {
  ...
  followingUsers(page) async {
    homeCon = videoRepo.homeCon.value;
    setState(() {});
    showLoader = true;
    scrollController = new ScrollController();
    followRepo.followingUsers(page, searchKeyword).then((userValue) {
      showLoader = false;
  ...
  followers(page) async {
    homeCon = videoRepo.homeCon.value;
    setState(() {});
    showLoader = true;
    scrollController = new ScrollController();
    followRepo.followers(page, searchKeyword).then((userValue) {
      showLoader = false;
      if (userValue.users.length == userValue.totalRecords) {
        showLoadMore = false;
      }
