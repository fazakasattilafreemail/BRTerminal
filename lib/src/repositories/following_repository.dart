...
Future<FollowingModel> followingUsers(page, searchKeyword) async {
  if (page == 1) {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        if (page > 1) {
          usersData.value.users.addAll(
              FollowingModel.fromJson(json.decode(response.body)['data'])
                  .users);
        } else {
          usersData.value =
              FollowingModel.fromJson(json.decode(response.body)['data']);
...
Future<FollowingModel> followers(page, searchKeyword) async {
  if (page == 1) {
    usersData.value = FollowingModel.fromJson({});
    usersData.notifyListeners();
  }
...
Future<FollowingModel> friendsList(page, searchKeyword) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        if (page > 1) {
          friendsData.value.users.addAll(
              FollowingModel.fromJson(json.decode(response.body)['data'])
                  .users);
        } else {
          friendsData.value =
