...
Future<String> followUnfollowUser(userId) async {
  ...
  final response = await client.post(
    ...
    headers: headers,
    body: json.encode(
        {"follow_by": userRepo.currentUser.value.userId.toString(), "follow_to": userId.toString(), "app_token": userRepo.currentUser.value.token}),
  );
...
Future<String> blockUser(userId) async {
  ...
  final response = await client.post(
    ...
    headers: headers,
    body: json.encode(
        {"blocked_by": userRepo.currentUser.value.userId.toString(), "user_id": userId.toString(), "app_token": userRepo.currentUser.value.token}),
  );
  if (response.statusCode == 200) {
    return json.encode(json.decode(response.body));
  } else {
...
Future<UserProfileModel> getUserProfile(userId, page) async {
  ...
  uri =
      uri.replace(queryParameters: {"user_id": userId.toString(), 'login_id': userRepo.currentUser.value.userId.toString(), 'page': page.toString()});
  try {
    Map<String, String> headers = {
      'Content-Type': 'application/json; charset=UTF-8',
      'USER': '${GlobalConfiguration().get('api_user')}',
...
Future<LoginData> getCurrentUser() async {
  ...
  if (currentUser.value.auth == null && prefs.containsKey('current_user')) {
    currentUser.value = LoginData.fromJson(json.decode(await prefs.get('current_user')));
