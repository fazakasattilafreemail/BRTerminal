class HashVideosModel {
  ...
  HashVideosModel.fromJson(Map<String, dynamic> jsonMap) {
    try {
      ...
      videos =
          jsonMap['data'] != null ? parseVideoAttributes(jsonMap['data']) : [];
      banners = jsonMap['tagBanners'] != null
          ? parseBannerAttributes(jsonMap['tagBanners'])
          : [];
    } catch (e) {
      totalRecords = 0;
      videos = [];
  ...
  static List<Banners> parseBannerAttributes(attributesJson) {
    List list = attributesJson;
    List<Banners> attrList =
        list.map((data) => Banners.fromJSON(data)).toList();
    return attrList;
  }
}
